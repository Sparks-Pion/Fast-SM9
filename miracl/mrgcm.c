...
void gcm_init(gcm *g, int nk, char *key, int niv,
              char *iv) { /* iv size niv is usually 12 bytes (96 bits). AES key size nk can be 16,24 or 32 bytes */
    int i;
    MR_BYTE H[16];
    for (i = 0; i < 16; i++) {
        H[i] = 0;
...
BOOL gcm_add_cipher(gcm *g, int mode, char *plain, int len,
                    char *cipher) { /* Add plaintext to extract ciphertext, or visa versa, depending on mode. len is length of plaintext/ciphertext. Note this file combines GHASH() functionality with encryption/decryption */
    int i, j = 0;
    MR_WORD counter;
    MR_BYTE B[16];
...
void gcm_finish(gcm *g, char *tag) { /* Finish off GHASH and extract tag (MAC) */
    int i;
