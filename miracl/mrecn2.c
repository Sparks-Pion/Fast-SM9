...
unsigned long ecn2_mul_cnt = 0;

void start_ecn2_mul_cnt()
{
    ecn2_mul_cnt = 0;
}

unsigned long get_ecn2_mul_cnt()
{
    return ecn2_mul_cnt;
}
...
BOOL ecn2_add_sub(_MIPD_ ecn2 *P, ecn2 *Q, ecn2 *PP,
...
int ecn2_muln_engine(_MIPD_ int wa, int sza, int wb, int szb, big *ma, big *ma3, big *mb, big *mb3, ecn2 *Ta, ecn2 *Tb,
...
void ecn2_mult4(_MIPD_ big *e, ecn2 *P, ecn2 *R) { /* R=e[0]*P[0]+e[1]*P[1]+ .... e[n-1]*P[n-1]   */
    int i, j, k, l, nb, ea, c;
    ecn2 G[16];
    zzn2 work[16];
#ifdef MR_OS_THREADS
