...
static double pairing_time = 0;
unsigned long long pairing_count = 0;
...
double end_count_time() {
    return pairing_time / pairing_count;
}
...
void line(_MIPD_ ecn2 *A, ecn2 *C, ecn2 *B, zzn2 *slope, zzn2 *extra, BOOL Doubling, zzn Qx, zzn Qy, zzn12 *wt) {
    ...
    if (Doubling) {
        ...
        if (mr_mip->TWIST == MR_SEXTIC_M) { // "multiplied across" by i to simplify
...
void endomorph(_MIPD_ ecn *A, zzn *Beta) { // apply endomorphism (x,y) = (Beta*x,y) where Beta is cube root of unity
    big x;
    x = mirvar(_MIPP_ 0);
    copy(A->X, x);
...
void
q_power_frobenius(_MIPD_ ecn2 *A, zzn2 *F) {// apply endomorphism (x,y) = (Beta*x,y) where Beta is cube root of unity
    zzn2 x, y, z, w, r, cj;
    zzn2_mirvar(_MIPP_ &x);
    zzn2_mirvar(_MIPP_ &y);
    zzn2_mirvar(_MIPP_ &z);
...
//    if (r.iszero()) return FALSE;
   ...
   */
    ...
    */
    ...
    t0*=res;
    */
    zzn12_mul(_MIPP_ &t0, &t0, &t0);// T_0 = y_6 * y_6
    zzn12_mul(_MIPP_ &t0, &x4, &t0);// T_0 = T_0 * y_4
    zzn12_mul(_MIPP_ &t0, &x5, &t0);// T_0 = T_0 * y_5

    zzn12_mul(_MIPP_ &x3, &x5, res);// T_1 = y_3 * y_5
    zzn12_mul(_MIPP_ res, &t0, res);// T_1 = T_1 * T_0
    zzn12_mul(_MIPP_ &t0, &x2, &t0);// T_0 = T_0 * y_2

    zzn12_mul(_MIPP_ res, res, res);// T_1 = T_1 * T_1
    zzn12_mul(_MIPP_ res, &t0, res);// T_1 = T_1 * T_0
    zzn12_mul(_MIPP_ res, res, res);// T_1 = T_1 * T_1

    zzn12_mul(_MIPP_ res, &x1, &t0);// T_0 = T_1 * y_1
    zzn12_mul(_MIPP_ res, &x0, res);// T_1 = T_1 * y_0
    zzn12_mul(_MIPP_ &t0, &t0, &t0);// T_0 = T_0 * T_0
    zzn12_mul(_MIPP_ &t0, res, &t0);// T_0 = T_0 * T_1

    zzn12_copy(&t0, res);// result = T_0
    ...
    finish = clock();
    double time = (double) (finish - start) / (double) CLOCKS_PER_SEC * 1000;
    pairing_time += time;
    pairing_count++;

    if(!pairing_cnt_print)
    {
        printf("\n===========pairing-count-begin===========\n");
